...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
assignments_B = IndexToAssignment(1:length(B.val), B.card);
...
for i = 1:length(B.val)
    idx_tmp=AssignmentToIndex(assignments_B(i,:),B.card);
    idx_to_sum_inA=find(indxB==idx_tmp);
    assignments_to_sum_inA=IndexToAssignment(idx_to_sum_inA,A.card);
    val=sum(GetValueOfAssignment(A,assignments_to_sum_inA));
    B=SetValueOfAssignment(B,assignments_B(i,:),val);
