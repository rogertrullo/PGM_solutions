...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
val_A=GetValueOfAssignment(A, IndexToAssignment(indxA, A.card));
val_B=GetValueOfAssignment(B, IndexToAssignment(indxB, B.card));

C = SetValueOfAssignment(C, assignments, val_A.*val_B);


% for i = 1:length(C.val)
%     val_A=GetValueOfAssignment(A, IndexToAssignment(indxA(i), A.card));
%     val_B=GetValueOfAssignment(B, IndexToAssignment(indxB(i), B.card));
%     C = SetValueOfAssignment(C, assignments(i,:), val_A*val_B);
%     
%     
% end
