...
for i = 1:size(E, 1),
    ...
    for j = 1:length(F),
        ...
        if (~isempty(indx)),
            ...
            %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
            [dummy, map_v] = ismember(v, F(j).var);
            assignments = IndexToAssignment(1:length(F(j).val), F(j).card);

            for k=1:length(F(j).val)
                val_factor=assignments(k,map_v);%column with the variable observed
                if val_factor ~= x%if evience does not agree with the value in the facto set it to 0
