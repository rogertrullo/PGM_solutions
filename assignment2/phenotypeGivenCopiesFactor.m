...
nalleles=size(allelesToGenotypes,1);
phenotypeFactor.var=[phenotypeVar,geneCopyVarOne, geneCopyVarTwo];
phenotypeFactor.card=[2,nalleles,nalleles];
...
for i=1:2:prod(phenotypeFactor.card)
    ...
    geno=allelesToGenotypes(assignments(i,2),assignments(i,3));
    

    phenotypeFactor= SetValueOfAssignment(phenotypeFactor, assignments(i:i+1,:), [alphaList(geno),1-alphaList(geno)]);
