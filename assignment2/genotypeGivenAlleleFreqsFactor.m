...
genotypeFactor.card=size(genotypesToAlleles,1);%nallels C 2 + nallels
...
for i = 1:length(genotypeFactor.val)
    a=genotypesToAlleles(i,1);
    b=genotypesToAlleles(i,2);
    
    if a==b
        genotypeFactor=SetValueOfAssignment(genotypeFactor, assignments(i,:), alleleFreqs(a)*alleleFreqs(b));
    else
        genotypeFactor=SetValueOfAssignment(genotypeFactor, assignments(i,:), 2*alleleFreqs(a)*alleleFreqs(b));
