...
for idpeople=1:numPeople
    ...
    if isequal(parents_matrix(idpeople,:),[0,0])%is a parent
        genotypeFactor = genotypeGivenAlleleFreqsFactor(alleleFreqs, idpeople);
        
        
    else
        genotypeFactor = genotypeGivenParentsGenotypesFactor(numAlleles, idpeople, parents_matrix(idpeople,1), parents_matrix(idpeople,2));
    end
    %second genotype
    phenotypeFactor = phenotypeGivenGenotypeFactor(alphaList, idpeople, numPeople+idpeople);
    factorList(idfactor)=genotypeFactor;
    factorList(idfactor+1)=phenotypeFactor;
    idfactor=idfactor+2;
end
