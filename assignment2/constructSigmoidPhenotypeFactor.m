...
ngenes=size(geneCopyVarOneList,1);
nalleles=size(alleleWeights{1},2);


phenotypeFactor.var=[phenotypeVar,geneCopyVarOneList', geneCopyVarTwoList'];%pheno|genesP1,genesP2
phenotypeFactor.card=[2,nalleles*ones(1,2*ngenes)];
...
for i=1:2:prod(phenotypeFactor.card)
    ...
    acum=0;
    for geneid=1:ngenes%for each gene:
        
        for alleleid=1:nalleles%for each allel:
            valP1=assignments(i,geneid+1);
            valP2=assignments(i,ngenes+geneid+1);
            
           acum=acum+(alleleWeights{geneid}(alleleid))*((valP1==alleleid)+(valP2==alleleid));           
            
        end     
        
    end
    
    prob_has_trait=computeSigmoid(acum);
    
    phenotypeFactor= SetValueOfAssignment(phenotypeFactor, assignments(i:i+1,:), [prob_has_trait,1-prob_has_trait]);
