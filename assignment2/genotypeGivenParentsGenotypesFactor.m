...
ngenotypes=size(genotypesToAlleles,1);%number of genoytpes (nallels C 2)+nallels

genotypeFactor.var=[genotypeVarChild, genotypeVarParentOne, genotypeVarParentTwo];
genotypeFactor.card=[ngenotypes,ngenotypes,ngenotypes];%child,P1,P2
...
for i = 1:length(genotypeFactor.val)
    geno_child=assignments(i,1);
    geno_P1=assignments(i,2);
    geno_P2=assignments(i,3);
    alleles_child=genotypesToAlleles(geno_child,:);
    alleles_P1=genotypesToAlleles(geno_P1,:);
    alleles_P2=genotypesToAlleles(geno_P2,:);    
    
    
    id_P1=find(alleles_P1==alleles_child(1));%number of coincidences with geno of parent1
    P_first_allel=length(id_P1)/2;
    
    id_P2=find(alleles_P2==alleles_child(2));%number of coincidences with geno of parent1
    P_second_allel=length(id_P2)/2;
    
    P_geno_child=P_first_allel*P_second_allel;
    
    if alleles_child(1)~=alleles_child(2)%if they are different eg. a,b we need to consider b,a
        alleles_child_opposite=[alleles_child(2),alleles_child(1)];
        id_P1=find(alleles_P1==alleles_child_opposite(1));%number of coincidences with geno of parent1
        P_first_allel=length(id_P1)/2;
    
        id_P2=find(alleles_P2==alleles_child_opposite(2));%number of coincidences with geno of parent1
        P_second_allel=length(id_P2)/2;
    
        P_geno_child=P_first_allel*P_second_allel+P_geno_child;%add the prev prob
        
    end
    
    
    
    genotypeFactor=SetValueOfAssignment(genotypeFactor, assignments(i,:), P_geno_child);
