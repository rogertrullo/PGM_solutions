...
tmp_factor=struct('var', [], 'card', [], 'val', []);
tmp_factor.card=[K,K,K];
tmp_factor.val=ones(prod(tmp_factor.card),1);
%assignments_tmp = IndexToAssignment(1:prod(tmp_factor.card), tmp_factor.card);

for idtriplet=1:length(tripletList)
    
    %I = AssignmentToIndex(tripletList(idtriplet).chars, tmp_factor.card)
    tmp_factor=SetValueOfAssignment(tmp_factor,tripletList(idtriplet).chars,tripletList(idtriplet).factorVal);
end
